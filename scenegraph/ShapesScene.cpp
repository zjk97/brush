...
#include "shapeSupport/OpenGLShape.h"
#include "shapeSupport/Cube.h"
//--------------------------------------------------
    ...
    m_vertices(0)
{
    ...
    //m_currentShape = std::make_unique<OpenGLShape>();
...
void ShapesScene::settingsChanged() {
    ...
    case 0:
        ...
        m_currentShape = std::make_unique<Cube>(settings.shapeParameter1, settings.shapeParameter2, settings.shapeParameter3);
        m_currentShape->setVertexData(m_currentShape->m_vertices.data(), m_currentShape->m_vertices.size(),
                                      VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, m_currentShape->m_vertices.size()/3);
        m_currentShape->setAttribute(ShaderAttrib::POSITION, 3, 0, VBOAttribMarker::DATA_TYPE::FLOAT, false);
        m_currentShape->setAttribute(ShaderAttrib::NORMAL, 3, 3*sizeof (float), VBOAttribMarker::DATA_TYPE::FLOAT, true);
        m_currentShape->buildVAO();
        break;
        /**
        ...
        m_currentShape->setVertexData(vertices.data(), vertices.size(),
                                      VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLE_STRIP, vertices.size()/3);
        m_currentShape->setAttribute(ShaderAttrib::POSITION, 3, 0, VBOAttribMarker::DATA_TYPE::FLOAT, false);
        m_currentShape->setAttribute(ShaderAttrib::NORMAL, 3, 3*sizeof (float), VBOAttribMarker::DATA_TYPE::FLOAT, true);
        m_currentShape->buildVAO();
        break;
    ...
    case 1:
        ...
        m_currentShape->setVertexData(vertices.data(), 9, VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLES, 3);
        m_currentShape->setAttribute(ShaderAttrib::POSITION, 3, 0, VBOAttribMarker::DATA_TYPE::FLOAT, false);
        m_currentShape->buildVAO();
        break;
        break;
